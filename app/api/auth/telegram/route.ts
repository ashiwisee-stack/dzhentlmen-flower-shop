...
export async function POST(request:Request) {
  try {
    ...
    if(p.action==="start") {
      ...
      return Response.json({url:`https://t.me/${bot.username}?start=auth_${token}`,code,expires:now+600000},{headers:{"set-cookie":challengeCookie(browser,request),"cache-control":"no-store"}});
    }
    if(!cookie) throw new Error("Начните вход заново в этом браузере");
    const hash=await sha256(cookie);
    if(p.action==="cancel") {await db.prepare("DELETE FROM telegram_auth WHERE browser_hash=?").bind(hash).run();return Response.json({ok:true},{headers:{"set-cookie":challengeCookie("",request,true)}});}
    ...
    const headers=new Headers({"cache-control":"no-store"});headers.append("set-cookie",makeCustomerCookie(session,request));headers.append("set-cookie",challengeCookie("",request,true));
