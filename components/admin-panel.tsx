...
const paymentStatus: Record<string, string> = { not_required: "При получении", pending: "Ожидает оплаты", paid: "Оплачен", refunded: "Возврат" };
...
export function AdminPanel({ userName }: { userName: string }) {
  ...
  const [paymentSetup,setPaymentSetup]=useState<{ready:boolean;test:boolean;hash:string;missing:string[];resultUrl:string;successUrl:string;failUrl:string}|null>(null);
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setPaymentSetup(dataPayload.paymentSetup||null);setIntegrations(dataPayload.integrations||{});setTotals(dataPayload.totals||{newOrders:0,customers:0,requests:0,turnover:0});
  ...
  return (
    <main className="admin-shell">
      ...
      <Dialog open={editorOpen} onOpenChange={setEditorOpen}><DialogContent className="product-editor"><DialogHeader><span className="eyebrow">Карточка товара</span><DialogTitle>{draft.id ? "Редактирование" : "Новый товар"}</DialogTitle><DialogDescription>Первое фото — главное. Фото могут быть вертикальными, горизонтальными или квадратными. Превью обрезается, оригинал сохраняется.</DialogDescription></DialogHeader><form className="product-editor-form" onSubmit={saveProduct}><label className="single-flower-toggle wide"><span><strong>Дополнительный товар</strong><small>Показывать в каталоге и в «Дополнительные товары». Цена за штуку.</small></span><Switch checked={draft.singleFlower===true} onCheckedChange={singleFlower=>{setDraft({...draft,singleFlower,acceptsFlowers:!singleFlower,category:singleFlower?(categories.find(c=>c.slug==="single-flowers")?.name||draft.category):draft.category});if(singleFlower)setHasSizes(false);}}/></label><label className="check-row"><input type="checkbox" checked={draft.acceptsFlowers!==false} onChange={e=>setDraft({...draft,acceptsFlowers:e.target.checked})}/>Разрешить дополнять этот товар</label><label className="check-row"><input type="checkbox" checked={draft.showRecommendations!==false} onChange={e=>setDraft({...draft,showRecommendations:e.target.checked})}/>Показывать «С этим товаром ещё берут»</label>
        ...
        <div className="pricing-mode wide"><label><span><strong>У товара есть размеры</strong><small>{draft.singleFlower?"Дополнительный товар продаётся по одной цене за штуку":"Выключите, чтобы задать одну цену без выбора размера"}</small></span><Switch checked={hasSizes} disabled={draft.singleFlower} onCheckedChange={checked=>{setHasSizes(checked);if(checked&&draft.variants.length===0)setDraft({...draft,variants:[{id:"s",name:"S",price:draft.price,available:draft.available}]});}}/></label></div>
        {!hasSizes && <label className="single-price wide"><span>{draft.singleFlower?"Цена за штуку, ₽":"Цена товара, ₽"}</span><input type="number" required min="0" max="1000000" step="1" value={draft.price} onChange={event=>setDraft({...draft,price:Number(event.target.value)})}/></label>}
