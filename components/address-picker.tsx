...
export function AddressPicker({ address, onAddress, onQuote, tileUrl, zone, children }: { address: string; onAddress: (value: string) => void; onQuote: (quote: Quote | null) => void; tileUrl: string; zone: number[][]; children?:ReactNode }) {
  ...
  return <div className="address-picker" ref={setPopupContainer}>
    {quote ? <><div className="delivery-confirmed"><strong>{quote.address}</strong><span>Доставка — {formatPrice(quote.price)}</span><Button type="button" variant="outline" onClick={()=>{invalidate();setEnabled(false);setPoint(null);setSelected(null);setResults([]);}}>Изменить адрес</Button></div>{quote.method === "road" && <small className="map-attribution">© OpenStreetMap · <a href="https://routing.openstreetmap.de/about.html" target="_blank" rel="noopener noreferrer">FOSSGIS</a> · <a href="https://www.openstreetmap.org/fixthemap" target="_blank" rel="noopener noreferrer">Исправить карту</a></small>}{children}</> : <>
