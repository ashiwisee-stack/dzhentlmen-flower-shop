...
export function Shop() {
  ...
  const [authMethods, setAuthMethods] = useState({ call: false, telegram: false });
  const [payment, setPayment] = useState({ ready: false, test: false });
  ...
  const [orderForm, setOrderForm] = useState({
    customerName: "", phone: "", fulfillment: "delivery", paymentMethod: "on_receipt", address: "", branchId: "kraulya",
  ...
  useEffect(() => {
    ...
    setCart(Array.isArray(savedCart)?savedCart.filter((l):l is CartLine=>!!l && typeof l==="object" && Number.isInteger(l.productId) && typeof l.variantId==="string" && typeof l.key==="string" && Number.isInteger(l.quantity) && l.quantity>0 && l.quantity<=999 && Array.isArray(l.extras) && l.extras.length<=500 && l.extras.every((x:unknown)=>typeof x==="string")):[]);
    setLocalReady(true);
    void customerFetch("/api/store").then((response) => response.json()).then((data) => {
      ...
      setAuthMethods(data.auth || {call:false,telegram:false});
      setPayment(data.payment || {ready:false,test:false});
  ...
  async function loadAccount(page=0) {
    try {
      const response = await customerFetch("/api/auth?page="+page);
  ...
  useEffect(() => {
    ...
    async function refresh() {
      ...
      try {
        const response=await customerFetch("/api/store",{cache:"no-store"});const data=await response.json();
        ...
        setAuthMethods(data.auth || {call:false,telegram:false});setPayment(data.payment || {ready:false,test:false});
  ...
  const cartAvailable=resolvedCart.length===cart.length && resolvedCart.every(l=>l.product.available && !l.product.hidden && categories.some(c=>c.name===l.product.category && c.visible) && l.variant.available && l.extras.every(id=>extras.some(e=>e.id===id && e.available!==false && (e.kind==="flower"?l.product.acceptsFlowers!==false:l.product.showRecommendations!==false))));
  ...
  function addToCart(product: Product, variantId?: string, selectedExtras: string[] = []) {
    ...
    setCart((current) => {
      ...
      return found ? current.map((item) => item.key === found.key ? { ...item, quantity: Math.min(999, item.quantity + 1) } : item) : [...current, { key:crypto.randomUUID(), productId: product.id, variantId: variant.id, quantity: 1, extras: selectedExtras }];
    });
    toast.success(`${product.name} добавлен в корзину`);
  }

  function setQuantity(key: string, quantity: number) {
    setCart((current) => quantity < 1 ? current.filter((item) => item.key !== key) : current.map((item) => item.key === key ? { ...item, quantity: Math.max(1,Math.min(999, Math.floor(quantity))) } : item));
  }

  async function logout() {
    await customerFetch("/api/auth", { method: "DELETE" });
    setCustomer(null);
    setAccountOrders([]);
  }
  ...
  async function createOrder(event: FormEvent) {
    ...
    try {
      ...
      const response=await customerFetch("/api/orders",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({...payload,requestKey:pending.key,accessToken:pending.token}),signal:AbortSignal.timeout(25000)});
  ...
  async function connectTelegram() {
    try {const r=await customerFetch("/api/telegram",{method:"POST",headers:{"content-type":"application/json"},body:"{}"}),d=await r.json();if(!r.ok)throw new Error(d.error);window.location.assign(d.url);}catch(e){toast.error(e instanceof Error?e.message:"Не удалось подключить");}
  }
  ...
  return (
    <main className="shop-shell">
      ...
      <footer className="shop-footer">
        ...
        <div><strong>Документы</strong><a href="/privacy">Политика конфиденциальности</a><a href="/offer">Публичная оферта</a><a href="/consent">Согласие на обработку данных</a></div>
      ...
      <Dialog open={Boolean(detailProduct)} onOpenChange={(open) => !open && setDetailProduct(null)}>
        <DialogContent className="product-dialog">{detailProduct && <div className="product-dialog-grid">
          ...
          <div className="detail-copy"><DialogHeader><span className="product-category">{detailProduct.category}</span><DialogTitle>{detailProduct.name}</DialogTitle><DialogDescription>{detailProduct.description}</DialogDescription></DialogHeader>
            ...
            {detailProduct.variants.length===1 && detailProduct.variants[0].id==="one" ? <div className="detail-block"><strong>{detailProduct.singleFlower?"Цена за штуку":"Цена"}</strong><p>{formatPrice(detailProduct.price)}</p></div> : <div className="detail-block"><strong>Размер</strong><div className="variant-grid">{detailProduct.variants.map((variant) => <button key={variant.id} disabled={!variant.available} className={detailVariant === variant.id ? "active" : ""} onClick={() => setDetailVariant(variant.id)}><span>{variant.name}</span><strong>{formatPrice(variant.price)}</strong>{!variant.available && <small>Нет</small>}</button>)}</div></div>}
            {extras.length > 0 && detailProduct.showRecommendations!==false && <div className="detail-block"><strong>С этим товаром ещё берут</strong><div className="extras-list">{extras.filter(e=>e.available!==false && e.kind!=="flower").map((extra) => <label key={extra.id}><input type="checkbox" checked={detailExtras.includes(extra.id)} onChange={() => setDetailExtras((current) => current.includes(extra.id) ? current.filter((id) => id !== extra.id) : [...current, extra.id])} /><span><b>{extra.name}</b><small>{extra.description}</small></span><strong>+{formatPrice(extra.price)}</strong></label>)}</div></div>}
      ...
      <Sheet open={cartOpen} onOpenChange={setCartOpen}>
        <SheetContent className="cart-sheet"><SheetHeader><SheetTitle>Корзина <span>{cartCount}</span></SheetTitle><SheetDescription>Проверьте состав заказа перед оформлением.</SheetDescription></SheetHeader>
          {cart.filter(line=>!resolvedCart.some(r=>r.key===line.key)).map(line=><div key={line.key} className="minimum-note">Этот товар или размер удалён из каталога. <Button variant="outline" onClick={()=>setQuantity(line.key,0)}>Убрать из корзины</Button></div>)}{resolvedCart.length ? <><div className="cart-lines">{resolvedCart.map((line) => <article key={line.key}><div className="cart-line-image"><Image src={line.product.image} alt="" fill sizes="82px" /></div><div className="cart-line-copy"><strong>{line.product.name}</strong>{line.variant.id!=="one"&&<span>Размер: {line.variant.name}</span>}{line.selectedExtras.length > 0 && <small>+ {line.selectedExtras.map((item) => item.name).join(", ")}</small>}<div className="quantity"><button onClick={() => setQuantity(line.key, line.quantity - 1)}><Minus /></button><QuantityInput value={line.quantity} onChange={n=>setQuantity(line.key,n)} min={1} max={999} label={"Количество: "+line.product.name}/><button onClick={() => setQuantity(line.key, line.quantity + 1)}><Plus /></button></div></div><div className="cart-line-price"><button onClick={() => setQuantity(line.key, 0)} aria-label="Удалить"><Trash2 /></button><strong>{formatPrice(line.unitPrice * line.quantity)}</strong></div><div className="cart-line-extras">{(!line.product.available || !line.variant.available || line.extras.some(id=>!extras.some(e=>e.id===id && e.available!==false))) && <p role="alert" className="form-error">Товар или дополнение недоступно. Измените состав.</p>}{line.extras.some(id=>extras.some(e=>e.id===id && (e.kind==="flower"?line.product.acceptsFlowers===false:line.product.showRecommendations===false))) && <div role="alert"><p>Для этого товара выбранные дополнения больше недоступны.</p><Button type="button" variant="outline" onClick={()=>changeExtras(line.key,[])}>Убрать дополнения</Button></div>}{line.product.acceptsFlowers!==false && <CartExtras extras={extras} selected={line.extras} flowers onChange={ids=>changeExtras(line.key,ids)}/>}{line.product.showRecommendations!==false && <CartExtras extras={extras} selected={line.extras} flowers={false} onChange={ids=>changeExtras(line.key,ids)}/>}</div></article>)}</div>
...
function CustomRequestForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    setLoading(true);
    try {
    const response = await customerFetch("/api/requests", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(form) });
